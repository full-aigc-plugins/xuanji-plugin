...
玄机插件（xuanji-plugin, Apache-2.0）收录，改编自 MIT 上游 jinchenma94/bazi-skill
（Copyright (c) 2025 jinchenma94），归属详见仓根 NOTICE。仅供娱乐。
...
def solar_to_lunar(y, m, d):
    ...
    raise ValueError(f"无法把 {y:04d}-{m:02d}-{d:02d} 换成农历（超出推算范围）")
...
def lunar_to_solar(ly, lm, ld, leap=False):
    ...
    for info in months:
        if info["year"] == ly and info["month"] == lm and bool(info["leap"]) == bool(leap):
            found = info
            break
    month_name = LUNAR_MONTH_NAMES.get(lm, str(lm) + "月")
    if found is None:
        if leap:
            raise ValueError(f"农历 {ly}年没有闰{month_name}")
        raise ValueError(f"找不到农历 {ly}年{month_name}")
    if ld < 1 or ld > found["days"]:
        leap_mark = "闰" if leap else ""
        raise ValueError(f"农历 {ly}年{leap_mark}{month_name}只有 {found['days']} 天")
    return found["start"] + timedelta(days=ld - 1)


def format_lunar(ly, lm, ld, leap):
    month_name = LUNAR_MONTH_NAMES.get(lm, str(lm) + "月")
    day_name = LUNAR_DAY_NAMES.get(ld, str(ld))
    leap_mark = "闰" if leap else ""
    return f"{ly}年{leap_mark}{month_name}{day_name}"
...
def format_canggan(zhi):
    ...
    return "、".join(f"{g}{wx}" for g, wx in parts)
...
def compute_shensha(
    ...
    def add(name, positions, method):
        if positions:
            lines.append("- {} — 见于{} — {}".format(name, "、".join(positions), method))
...
def build_dayun(month_gan, month_zhi, forward, qiyun_years, qiyun_months, steps=8):
    ...
    if qiyun_years > 0 or qiyun_months > 0:
        if qiyun_months == 0:
            pre_end = f"{qiyun_years}岁"
        else:
            pre_end = f"{qiyun_years}岁{qiyun_months}个月"
    ...
    for n in range(1, steps + 1):
        ...
        a0 = start_age + (n - 1) * 10
        a1 = a0 + 9
        rows.append((str(n), f"{a0}-{a1}岁", GAN[gi] + ZHI[zi]))
    return rows
...
def compute(
    ...
    month_gan, month_zhi, jie_name, _jie_lon, prev_jie_dt, next_jie_dt = resolve_month_pillar(
        birth_dt, GAN.index(year_gan)
    )

    if clock_known:
        d_gan, d_zhi, _, _, zishi = resolve_day_pillar(solar_date, hour, minute)
    elif shichen_known and shichen == "子":
        d_gan, d_zhi, _, _, zishi = resolve_day_pillar(solar_date, 0, 0)
        warnings.append("未提供具体时刻，子时按早子时（当日日柱）处理")
    else:
        d_gan, d_zhi, _d_idx, _day_used, zishi = resolve_day_pillar(solar_date, None, None)
    ...
    if abs((birth_dt - prev_jie_dt).total_seconds()) <= 86400 and "节气交界" not in warnings:
        warnings.append("节气交界")
    ...
    if clock_known:
        solar_text = f"{solar_date.year:04d}-{solar_date.month:02d}-{solar_date.day:02d} {hour:02d}:{minute:02d}"
        shichen_text = f"{shichen} / {hour:02d}:{minute:02d}"
    elif shichen_known:
        solar_text = f"{solar_date.year:04d}-{solar_date.month:02d}-{solar_date.day:02d}"
...
def format_report(result):
    ...
    lines = []
    lines.append("## 输入")
    lines.append("- 阳历：{}".format(result["solar_text"]))
    lines.append("- 农历：{}".format(result["lunar_text"]))
    lines.append("- 时辰：{}".format(result["shichen_text"]))
    lines.append("- 性别：{}".format(result["sex"]))
    if result["place"]:
        lines.append("- 出生地：{}".format(result["place"]))
    lines.append("- 子时：{}".format(result["zishi"]))
    lines.append("")
    lines.append("## 四柱")
    lines.append("| | 年柱 | 月柱 | 日柱 | 时柱 |")
    lines.append("|---|---|---|---|---|")
    lines.append(
        "| 天干 | {} | {} | {} | {} |".format(p["year"][0], p["month"][0], p["day"][0], p["hour"][0])
    )
    lines.append(
        "| 地支 | {} | {} | {} | {} |".format(p["year"][1], p["month"][1], p["day"][1], p["hour"][1])
    )
    lines.append(
        "| 十神 | {} | {} | {} | {} |".format(p["year"][2], p["month"][2], p["day"][2], p["hour"][2])
    )
    lines.append(
        "| 藏干 | {} | {} | {} | {} |".format(p["year"][3], p["month"][3], p["day"][3], p["hour"][3])
    )
    lines.append("")
    lines.append("## 大运")
    lines.append(f"- 方向：{'顺排' if result['forward'] else '逆排'}")
    lines.append("- 起运：{}".format(result["qiyun_text"]))
    lines.append("| 大运序 | 年龄范围 | 干支 |")
    lines.append("|---|---|---|")
    for seq, ages, gz in result["dayun_rows"]:
        lines.append(f"| {seq} | {ages} | {gz} |")
    lines.append("")
    lines.append("## 流年")
    lines.append(f"- 当前：{result['current_year']}年{result['current_gz']}")
    for y in result["liunian_years"]:
        lines.append(f"- {y}年{liunian_gz(y)}")
    lines.append("")
    lines.append("## 神煞")
    ...
    lines.append("## 免责声明")
    lines.append(
        "- 本结果为传统术数的娱乐化演绎，仅供娱乐，不构成任何医疗、法律、投资"
        "或其他专业建议；如有现实需求，请咨询相关专业人士。"
    )
    lines.append("")
    return "\n".join(lines)
...
def parse_iso_date(text, flag):
    try:
        ...
        raise argparse.ArgumentTypeError(f"{flag} 需要 YYYY-MM-DD，收到 {text!r}")
...
def parse_hour(text):
    try:
        ...
        raise argparse.ArgumentTypeError(f"--hour 需要 HH:MM，收到 {text!r}")
